...
Public submissions and guestbook entries require Cloudflare Turnstile verification before persistence on the live `oddweb.page` application. Configure production's public `TURNSTILE_SITEKEY`, exact `TURNSTILE_HOSTNAMES=oddweb.page` allowlist, and Worker secret `TURNSTILE_SECRET`. Local and staging environments do not load Turnstile and protected submissions fail closed there. Both live forms use separate widget instances and stable actions, `site_submission` and `guestbook`. Invalid, expired, replayed, wrong-action, wrong-hostname, and unavailable verification results fail closed. Public mutations use layered sliding-window limits designed for CGNAT prevalence: signed anonymous identity, HMAC-derived exact IP, coarse network bucket, and global caps. Site submissions allow 6/3h per identity, 24/3h per exact IP, 120/3h per network, and 300/3h globally. Guestbook entries allow 3/day, 12/day, 80/day, and 500/day across those scopes. Failed persistence refunds the reservation. Responses carry `429` with `Retry-After`; invalid challenges use `403` and verifier outages use `503`.

All public forms feature hidden honeypot fields (`homepage_hp`, `message_hp`) that immediately block automated bots without consuming Turnstile API verification calls. Public mutation endpoints enforce browser client integrity, blocking known automated scraper user agents (`curl`, `python-requests`, `Go-http-client`, `Postman`, `Scrapy`, etc.).

Votes are 1-click and Turnstile-free by default. They use a one-year, first-party, HttpOnly signed anonymous identity cookie and an HMAC-derived vote key. Each identity can change vote state 30 times/hour and 200 times/day, with exact-IP, coarse-network, global, and idempotency controls. If multiple active votes for the same site originate from the same IP network (e.g. browser switching or shared CGNAT) OR if a site undergoes a sudden viral velocity surge ($\ge 20$ votes in 10 minutes), stepped Turnstile verification is dynamically required. This immediately blocks automated bot swarms and voting scripts while allowing legitimate human voters during viral traffic spikes to verify their vote instantly into the public tally.

Scheduled maintenance crons automatically prune expired rate-limit reservations, stale turnstile failures, old vote toggle actions, and inactive identity records.
...
The generated config is `.wrangler/staging.jsonc` and targets the production build output created by `npm run staging:verify`. It records the current Git SHA and generation time. Wrangler cannot set required secrets before a Worker exists, so the first deployment must supply all four staging secrets atomically through a secured `.env` or JSON file outside the repository. The file must be readable only by its owner and define `ADMIN_USERNAME`, `ADMIN_PASSWORD_HASH`, `ADMIN_SESSION_SECRET`, and a 32-byte base64url `TAXONOMY_MASTER_KEY_V1`. Turnstile is live-only and is not configured for staging:
...
`staging:deploy:first` validates all four required secret keys before remote work, runs staging verification, requires a clean worktree, hashes the config, server, client, and secrets, and dry-runs the exact strict `--secrets-file` deployment before any mutation. It rechecks cleanliness and the digest before applying isolated D1 migrations and again before deployment, then validates handlers/consumer settings and smoke-tests the exact staging release SHA. Later staging releases run `npm run staging:smoke` against the staged Worker. Protected site and guestbook submissions are intentionally unavailable outside the live application. Use the ordinary staging verification and deployment controls for later changes; never commit or place the secrets file under the repository.
